...
if (User::userIsLoggedIn()) {
    ...
    $freight = Customs::getInquiryCustomsDetail($_REQUEST['id'], $user->UserId);
    ...
    if ($freight->status == 200 && !empty($freight->response[0])) {
        ...
        $customs_values = $freight->response;
        ...
        $sourceOutput = $freight->freight_customs;
//        if (!empty($freight->freight_customs)) {
//            $temp = Customs::getSourceInfoForCustoms($freight->freight_customs);
//            $city_name = array_column(json_decode($temp->response[0]->city_name), 'value', 'slug')[$_COOKIE['language']];
//            $country_name = array_column(json_decode($temp->response[0]->country_name), 'value', 'slug')[$_COOKIE['language']];
//            $c_name = array_column(json_decode($temp->response[0]->customs_name), 'value', 'slug')[$_COOKIE['language']];
//            $sourceOutput = $c_name . ' ' . $city_name . ' ' . $country_name . ' ';
//        }

        if ($freight->freight_status == 'completed') {
            Customs::updateInquiryStatus($freight->freight_id, 'read');
        }


        $transportation = Customs::getTransportationsByStatus($freight->transportation_id)->response[0]->category_name;
        ?>
                            ...
                            <div class="d-flex align-items-center mb-3">
                                <div class="mj-d-cargo-item-category me-2"
                                    ...
                                    <img src="/dist/images/icons/storage/warehouse(white).svg"
                                         alt="<?= $lang['u_inquery_customs_name_prefix'] ?>">
                                    <span> <?= $lang['u_inquery_customs_category_name_replace'] ?></span>
                                </div>
                                <div class="flex-fill">
                                    <h2 class="mj-d-cargo-item-header mt-0 mb-2"><?= $lang['u_inquery_customs_name_prefix'] . ' ' . $freight->freight_id ?></h2>
                                ...
                                <div class="cargo-detail col-6">
                                    <div class="d-flex align-items-center mb-3">
                                        <img src="/dist/images/icons/air/calendar-star.svg"
                                             ...
                                             alt="loading-time"/>
                                        <div>
                                            <div class="mj-d-cargo-item-title"><?= $lang['u_customs_date'] ?>:
                                            </div>
                                ...
                                <div class="cargo-detail col-6">
                                    <div class="d-flex align-items-center mb-3">
                                        <img src="/dist/images/icons/air/weight-scale.svg"
                                             ...
                                             alt="weight"/>
                                        <div>
                                            <div class="mj-d-cargo-item-title"><?= $lang['u_customs_transportation'] ?>
                                                :
                                            </div>
                                            <div class="mj-d-cargo-item-value">
                                                <?= array_column(json_decode($transportation), 'value', 'slug')[$_COOKIE['language']]; ?></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="cargo-detail col-6">
                                    <div class="d-flex align-items-center mb-3">
                                        <img src="/dist/images/icons/air/calendar-star.svg"
                                             ...
                                             alt="duration day"/>
                                        <div>
                                            <div class="mj-d-cargo-item-title"><?= $lang['u_customs_start_date'] ?>:
                                            </div>
                                            <div class="mj-d-cargo-item-value">
                                                <?= ($_COOKIE['language'] == 'fa_IR') ? Utils::jDate('Y/m/d', $freight->freight_submit_date) : date('Y-m-d', $freight->freight_submit_date) ?></div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card-body">
                                <div class="d-flex align-items-center mb-3">
                                    <img src="/dist/images/icons/boxes(blue).svg" class="mj-b-icon-box me-2"
                                         alt="description">
                                    <span class="mj-b-icon-title"><?=$lang['u_customs_cargo_lists']?></span>
                                </div>
                                <div class="mj-cargo-item-list">
                                    <?php foreach ($customs_values as $item) {
//      
                                    ?>
                                    <div class="mj-cargo-item   mb-1">
                                        <div class="fa-boxes"></div>
                                        <div class="mj-cargo-item-cat"><?=array_column(json_decode($item->category_name), 'value', 'slug')[$_COOKIE['language']];?></div>
                                        <div class="mj-cargo-item-name"><?=$item->value_name?></div>
                                        <div class="mj-cargo-item-hscode">
                                            <span><?=$lang['u_customs_cargo_code']?> :</span>
                                            <span><?=$item->value_hscode?></span>
                                        </div>
                                        <div class="mj-cargo-item-weight">
                                            <span><?=$lang['u_customs_cargo_weight']?> :</span>
                                            <div><?=$item->value_weight?> <span id="cargo-weight"><?=$item->value_weight_slug?></span></div>
                                        </div>
                                        <div class="mj-cargo-item-volume">
                                            <span><?=$lang['u_customs_cargo_volume']?> :</span>
                                            <div><?=$item->value_volume?> <span id="cargo-volume"><?=$lang['u_customs_cargo_volume_unit']?></span></div>
                                        </div>
                                    </div>
                                        <?php
                                    } ?>
