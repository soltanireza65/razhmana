...
if (User::userIsLoggedIn()) {
    ...
    if ($user->UserAuthStatus == 'accepted') {
        ...
        enqueueScript('withdraw-js', '/dist/js/user/wallet/withdraw.js');
    } else {
        ...
        <section style="padding-top: 110px">
            <div class="mj-wallet-head-blue">
                ...
                <div class="mj-wallet-rial-balance-withdraw">
                    ...
                    <img src="<?= Utils::getCurrencyImage((int)$_REQUEST['currencyId'])->icon; ?>" alt="">
        ...
        <section class="mj-witdraw-section">
            <div class="mj-accounts-list">
                ...
                foreach ($creditCards->response as $card) {
                    if ($card->CurrencyId == (int)$_REQUEST['currencyId'] && in_array($card->CardStatus, ['accepted'])) {
                        $flagEmpty = false;
                        ?>
                        <div class="mj-account-card mb-2">
                            <div class="mj-account-card-title">
                                <div class="mj-account-info">
                                    <div><?= $card->CardBankName; ?></div>
                                    <div>
                            ...
                            <div class="mj-withdraw-input">
                                <label for="withdraw-input"><?= $lang['wallet_withdraw_amount']; ?></label>
                                <div class="mj-withdraw-input-elem">
                                    <input type="text"
                                           inputmode="decimal"
                                           name="withdraw-input"
                                        <?= (in_array($card->CardStatus, ['accepted', 'pending'])) ? '' : 'disabled'; ?>
                                           placeholder="<?= $lang['u_wallet_withdraw_amount_placeholder']; ?>">
                                    <span><?= $BalanceCurrency; ?></span>
                                </div>
                                <button type="button"
                                        data-tj-currency="<?= (int)$_REQUEST['currencyId']; ?>"
                                        data-tj-credit="<?= $card->CardId; ?>"
                                    <?= (in_array($card->CardStatus, ['accepted', 'pending'])) ? '' : 'disabled'; ?>
                                        class="mj-withdraw-main-btn">
                                    <?= $lang['wallet_withdraw'] ?>
                                </button>
                            </div>
                        </div>
            ...
            <div class="mj-add-account-btn mt-4">
                <a href="/user/wallet/accounts/<?= (int)$_REQUEST['currencyId']; ?>">
                ...
                <div class="modal-content mj-modal-add-account">
                    <div class="modal-header mj-modal">
                        ...
                        <h1 class="modal-title fs-5"
                            id="exampleModalLabel"><?= $lang['d_add_credit_card_title']; ?></h1>
                    </div>
                    ...
                    <div class="modal-footer mj-modal-footer">
                        <button type="button"
                                ...
                                name="submit-card"
                                data-tj-currency="<?= (int)$_REQUEST['currencyId']; ?>"
                                class="btn btn-primary mj-add-account-button"><?= $lang['d_cargo_additional_expenses_button']; ?></button>
                    </div>
                </div>
            </div>
        </div>
