...
if (User::userIsLoggedIn()) {
    ...
    if ($cargo->status == 200) {
        ...
        if (!in_array($cargo->CargoStatus, ['pending', 'canceled', 'rejected', 'expired'])) {
            ...
            enqueueScript('send-request-in', '/dist/js/driver/send-request-in.js');
            ...
            <main class="container" style="padding-bottom: 80px;">
                <?php
                if (!Driver::checkCanSendRequestIn($cargo->CargoId, $user->UserId)) {
                    ?>
                    <div class="row">
                        <div class="col-12">
                ...
                if ($user->UserStatus != 'suspend' && Driver::checkCanSendRequestIn($cargo->CargoId, $user->UserId)) {
                    ?>
                    <div class="row">
                        <div class="col-12">
                                            ...
                                            <div class="mj-custom-select car-type">
                                                <select class="is-invalid"
                                                    ...
                                                    foreach ($carTypes->response as $item) {
                                                        ?>
                                                        <option data-image="<?= $item->TypeImage; ?>"
            ...
            header('location:/cargo-in/' . $cargo->CargoId);
        }
        getFooter('', false);
    } else {
        Router::trigger404();
