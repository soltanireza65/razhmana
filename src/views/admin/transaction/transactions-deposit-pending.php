...
// end roles 1
         ...
         */
        $Result = Transactions::getAllTransactionsByStatus('pending_deposit');
        $Data = [];
        if ($Result->status == 200 && !empty($Result->response)) {
            $Data = $Result->response;
        }

        getHeader($lang["deposit_pending"], [
            'lang' => $lang,
                ...
                if (!empty($Data)) {
                    foreach ($Data as $DataITEM) {
                        ...
                        if ($resultUserInfoById->status == 200 && !empty($resultUserInfoById->response) && !empty($resultUserInfoById->response[0])) {
                            ...
                            $type = $resultUserInfoById->response[0]->user_type;
                        }
                        ...
                        <div class="col-md-4">
                            <div class="card mb-0 mt-3 border border-warning">
                                <div class="card-body">
                                    <span class="badge badge-soft-warning float-end"><?= $lang['pending_deposit']; ?></span>
                                    <h5 class="mt-0">
                                        <a href="/admin/transaction/info/<?= $DataITEM->transaction_id; ?>"
                                           ...
                                           class="text-warning">
                                            <?= $DataITEM->transaction_authority; ?>
                                        </a>
                                    </h5>
