...
// end roles 1
         ...
         * Get All Cars
         */
        $Result = Cargo::getDriverRequestsByUserID($id);
        $Data = [];
         ...
         * Get All Cities
         */
        $dataAllCities = Ground::getAllCustomSomeValues()->response;
                                    ...
                                    if (!empty($Data)) {
                                        ...
                                        foreach ($Data as $dataAllRequestsITEM) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php
                                            if($dataAllRequestsITEM->cargo_green=="yes") {
                                                echo "<span class='text-success'>" . $lang["a_green_street_customs"] . "</span>";
                                            }else {
                                                $name = $lang['no_address'];
                                                if (!empty($dataAllCities)) {
                                                    foreach ($dataAllCities as $dataAllCitiesITEM) {
                                                        if ($dataAllCitiesITEM->customs_id == $dataAllRequestsITEM->cargo_origin_customs_id) {
                                                            $name = (!empty(array_column(json_decode($dataAllCitiesITEM->customs_name, true), 'value', 'slug')[$language])) ?
                                                                array_column(json_decode($dataAllCitiesITEM->customs_name, true), 'value', 'slug')[$language] : $lang['no_name'];
                                                            break;
                                                        }
                                                    }
                                                }
                                                ...
                                                <td>
                                                    ...
                                                    if (!empty($dataAllCities)) {
                                                        foreach ($dataAllCities as $dataAllCitiesITEM) {
                                                            if ($dataAllCitiesITEM->customs_id == $dataAllRequestsITEM->cargo_destination_customs_id) {

                                                                $name2 = (!empty(array_column(json_decode($dataAllCitiesITEM->customs_name, true), 'value', 'slug')[$language])) ?
                                                                    array_column(json_decode($dataAllCitiesITEM->customs_name, true), 'value', 'slug')[$language] : $lang['no_name'];
                                                                break;
                                                            }
                                                        }
