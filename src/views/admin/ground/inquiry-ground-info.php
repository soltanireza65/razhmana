...
// end roles 1
         ...
         */
        $result = Ground::getInquiryInfoById($id);
        $data = [];
        if ($result->status == 200) {
            $data = $result->response[0];
        }
        if (empty($data)) {
            header('Location: /admin/inquiry/ground');
        }
//print_r($data);
        /**
         * Get User Info By Id
         ...
         * Get Customs Name
         */
        $resultUsCustomsInfo = Ground::getMultiCityAndCustomsId("$data->source_customs_id ,$data->dest_customs_id");
        $customsInfo = [];
        if ($resultUsCustomsInfo->status == 200 && !empty($resultUsCustomsInfo->response)) {
            $customsInfo = $resultUsCustomsInfo->response;
        }

        /**
         * Get Cargo Category
         */
        $resultCargoCategory = Ground::getCargoCategory();
        $cargoCategoryInfo = [];
         ...
         * Get Cars Types
         */
        $resultCarsTypes = Ground::getCarsTypes();
        $carsTypesInfo = [];
        if ($resultCarsTypes->status == 200 && !empty($resultCarsTypes->response)) {
            $carsTypesInfo = $resultCarsTypes->response;
        }
        ...
        enqueueScript('inquiry-ground-info', '/dist/js/admin/ground/inquiry-ground-info.init.js');

        getHeader($lang["inquiry_ground_title"], [
            'lang' => $lang,
        ...
        if ($flagSlug) {
            ...
            <div class="row">
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tr>
                                        <td><?= $lang["a_requested_car"]; ?> :</td>
                                        <td>
                                                  <span id="change_inquiry_type"
                                                        data-type="select"
                                                        data-bs-toggle="tooltip"
                                                        data-bs-placement="top"
                                                        title="<?= $lang['click_for_edit']; ?>"
                                                        data-tj-type="car_type_id">
                                                         <?php
                                                         $array_type = [];
                                                         if (!empty($carsTypesInfo)) {
                                                             foreach ($carsTypesInfo as $loop) {
                                                                 if ($loop->type_id == $data->car_type_id) {
                                                                     echo (!empty(array_column(json_decode($loop->type_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                                         array_column(json_decode($loop->type_name, true), 'value', 'slug')[$_COOKIE['language']] : $data->car_type_id;
                                                                 }

                                                                 // from change and update to x-table
                                                                 $Car_name = (!empty(array_column(json_decode($loop->type_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                                     array_column(json_decode($loop->type_name, true), 'value', 'slug')[$_COOKIE['language']] : $loop->type_id;
                                                                 array_push($array_type, ["text" => $Car_name, 'value' => $loop->type_id]);
                                                             }
                                                         }
                                                         ?>
                                                  </span>
                                                ...
                                                <li class="align-items-center d-flex justify-content-around list-group-item">
                                                    ...
                                                    <span>
                                                         ...
                                                         if (!empty($customsInfo)) {
                                                             foreach ($customsInfo as $loop) {
                                                                 if ($loop->customs_id == $data->source_customs_id) {
                                                                     echo (!empty(array_column(json_decode($loop->customs_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                                         array_column(json_decode($loop->customs_name, true), 'value', 'slug')[$_COOKIE['language']] : $data->source_customs_id;
                                                                 }
                                                             }
                                                         }
                                                         ?>
                                            ...
                                            <ul class="list-group list-group-flush">
                                                ...
                                                <li class="align-items-center d-flex justify-content-around list-group-item">
                                                    ...
                                                    <span>
                                                          ...
                                                          if (!empty($customsInfo)) {
                                                              foreach ($customsInfo as $loop) {
                                                                  if ($loop->customs_id == $data->dest_customs_id) {
                                                                      echo (!empty(array_column(json_decode($loop->customs_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                                          array_column(json_decode($loop->customs_name, true), 'value', 'slug')[$_COOKIE['language']] : $data->dest_customs_id;
                                                                  }
                                                              }
                                                          }
                                                          ?>
                ...
                <div class="col-sm-12 col-md-12 col-lg-5">
                    ...
                    <?php
                    if ($data->freight_status != "completed") {
                        ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span id="add_inquiry_admin_description"
                                                  ...
                                                  data-bs-placement="top"
                                                  title="<?= $lang['click_for_edit']; ?>"
                                                  data-tj-type="freight_admin_description">
                                            </span>
                                        </td>
                                    </tr>
