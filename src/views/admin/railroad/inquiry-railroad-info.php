...
// end roles 1
         ...
         */
        $result = Railroad::getInquiryInfoById($id);
        $data = [];
        if ($result->status == 200) {
            $data = $result->response[0];
        }
        if (empty($data)) {
            header('Location: /admin/inquiry/railroad');
        }
         ...
         * Get railroad Name
         */
        $resultStation = Railroad::getMultiPortsByIDs("$data->source_railroad_id ,$data->dest_railroad_id");
        $dataStation = [];
         ...
         * Get Cargo Category
         */
        $resultCargoCategory = Railroad::getAllCategoryRailroadCargoFromTable('active');
        $cargoCategoryInfo = [];
         ...
         * Get All Container
         */
        $resultContainer = Railroad::getAllContainer('active');
        $dataContainer = [];
         ...
         * Get All wagon
         */
        $resultWagon = Railroad::getAllWagon('active');
        $dataWagon = [];
        if ($resultWagon->status == 200 && !empty($resultWagon->response)) {
            $dataWagon = $resultWagon->response;
        }

        /**
         * Get All Packing
         */
        $resultPacking = Railroad::getAllCategoryRailroadPackingActive();
        $dataPacking = [];
        ...
        enqueueScript('inquiry-railroad-info', '/dist/js/admin/railroad/inquiry-railroad-info.init.js');

        getHeader($lang["inquiry_railroad_title"], [
            'lang' => $lang,
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tr>
                                        <td><?= $lang["a_requested_wagon"]; ?> :</td>
                                        <td>
                                                  <span id="change_inquiry_wagon"
                                                        data-type="select"
                                                        data-bs-toggle="tooltip"
                                                        data-bs-placement="top"
                                                        title="<?= $lang['click_for_edit']; ?>"
                                                        data-tj-type="wagon_id">
                                                         <?php
                                                         $array_wagon = [];
                                                         if (!empty($dataWagon)) {
                                                             foreach ($dataWagon as $loop) {
                                                                 if ($loop->wagon_id == $data->wagon_id) {
                                                                     echo (!empty(array_column(json_decode($loop->wagon_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                                         array_column(json_decode($loop->wagon_name, true), 'value', 'slug')[$_COOKIE['language']] : $data->wagon_id;
                                                                 }

                                                                 // from change and update to x-table
                                                                 $wagon_name = (!empty(array_column(json_decode($loop->wagon_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                                     array_column(json_decode($loop->wagon_name, true), 'value', 'slug')[$_COOKIE['language']] : $loop->wagon_id;
                                                                 array_push($array_wagon, ["text" => $wagon_name, 'value' => $loop->wagon_id]);
                                                             }
                                                         }
                                                         ?>
                                                  </span>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            ...
                                            <ul class="list-group list-group-flush">
                                                ...
                                                <li class="align-items-center d-flex justify-content-around list-group-item">
                                                    <?= $lang['a_source_railroad']; ?>
                                                    <span>
                                                         <?php
                                                         if (!empty($dataStation)) {
                                                             foreach ($dataStation as $loop) {
                                                                 if ($loop->railroad_id == $data->source_railroad_id) {
                                                                     echo (!empty(array_column(json_decode($loop->railroad_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                                         array_column(json_decode($loop->railroad_name, true), 'value', 'slug')[$_COOKIE['language']] : $data->source_railroad_id;
                                                                 }
                                                             }
                                                         }
                                                         ?>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            ...
                                            <ul class="list-group list-group-flush">
                                                ...
                                                <li class="align-items-center d-flex justify-content-around list-group-item">
                                                    <?= $lang['a_dest_railroad']; ?>
                                                    <span>
                                                          <?php
                                                          if (!empty($dataStation)) {
                                                              foreach ($dataStation as $loop) {
                                                                  if ($loop->railroad_id == $data->dest_railroad_id) {
                                                                      echo (!empty(array_column(json_decode($loop->railroad_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                                          array_column(json_decode($loop->railroad_name, true), 'value', 'slug')[$_COOKIE['language']] : $data->dest_railroad_id;
                                                                  }
                                                              }
                                                          }
                                                          ?>
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tr>
                                        <td><?= $lang["a_freight_discharge"]; ?> :</td>
                                        <td>
                                            <span id="add_inquiry_discharge"
                                                  data-type="select"
                                                  data-bs-toggle="tooltip"
                                                  data-bs-placement="top"
                                                  title="<?= $lang['click_for_edit']; ?>"
                                                  data-tj-type="freight_discharge">
                                            <?= ($data->freight_discharge == "yes") ? $lang['a_yes'] : $lang['a_no']; ?>
                                            </span>
                                        </td>
                                    </tr>

                                    <tr>
                                        <td><?= $lang["a_freight_international_code"]; ?> :</td>
                                        <td>
                                            <span id="add_inquiry_international_code"
                                                  data-type="text"
                                                  data-bs-toggle="tooltip"
                                                  data-bs-placement="top"
                                                  title="<?= $lang['click_for_edit']; ?>"
                                                  data-tj-type="freight_international_code">
                                            <?= $data->freight_international_code; ?>
                                            </span>
                                        </td>
                                    </tr>
