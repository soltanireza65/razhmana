...
// end roles 1
         ...
         * Get All Reports
         */
        $result = PosterC::getAllReportFromTabel();
        $data = [];
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="row justify-content-between mb-3">
                                <div class="col-auto">
                                    <h4 class="page-title"><?= $lang['a_list_reports']; ?></h4>
                                </div>
                                <div class="col-auto">
                                    <div class="text-lg-end my-1 my-lg-0">
                                        <a target="_self" href="/admin/category/report/add"
                                    ...
                                    if (!empty($data)) {
                                        ...
                                        foreach ($data as $loop) {
                                            ...
                                            <tr>
                                                <td><?= $i++; ?></td>
                                                <td>
                                                    <?= (!empty(array_column(json_decode($loop->report_title, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                        array_column(json_decode($loop->report_title, true), 'value', 'slug')[$_COOKIE['language']] : $loop->report_id; ?>
                                                </td>
                                                <td>
                                                    <?= $loop->report_priority; ?>
                                                </td>
                                                <td>
                                                    <?php
                                                    if ($loop->report_status == "active") {
                                                        echo "<span class='badge badge-soft-success font-12'>" . $lang['active'] . "</span>";
                                                        $status_active += 1;
                                                    } elseif ($loop->report_status == "inactive") {
                                                        echo "<span class='badge badge-soft-warning font-12'>" . $lang['inactive'] . "</span>";
                                                        $status_inactive += 1;
                                                    } elseif ($loop->report_status == "user") {
                                                        echo "<span class='badge badge-soft-info font-12'>" . $lang['a_user_creator'] . "</span>";
                                                        $status_user += 1;
                                                    } else {
                                                        echo "<span class='badge badge-soft-pink font-12'>" . $loop->report_status . "</span>";
                                                    }
                                                    ?>
                                                </td>
                                                <td>
                                                    <a target="_self"
                                                       href="/admin/category/report/edit/<?= $loop->report_id; ?>"
