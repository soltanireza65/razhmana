...
// end roles 1
         ...
         * Get All Models
         */
        $result = PosterC::getAllModelsFromTabel();
        $data = [];
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="row justify-content-between mb-3">
                                <div class="col-auto">
                                    <h4 class="page-title"><?= $lang['a_list_models']; ?></h4>
                                </div>
                                <div class="col-auto">
                                    <div class="text-lg-end my-1 my-lg-0">
                                        <a target="_self" href="/admin/category/model/add"
                                    ...
                                    if (!empty($data)) {
                                        ...
                                        foreach ($data as $loop) {
                                            ...
                                            <tr>
                                                <td><?= $i++; ?></td>
                                                <td>
                                                    <?= (!empty(array_column(json_decode($loop->model_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                        array_column(json_decode($loop->model_name, true), 'value', 'slug')[$_COOKIE['language']] : $loop->model_id; ?>
                                                </td>
                                                <td>
                                                    <?php
                                                    if ($loop->brand_type == "truck") {
                                                        $type = "<span class='badge badge-outline-info font-12'>" . $lang['a_truck'] . "</span>";
                                                    } elseif ($loop->brand_type == "trailer") {
                                                        $type = "<span class='badge badge-outline-primary font-12'>" . $lang['a_trailer'] . "</span>";
                                                    } else {
                                                        $type = "<span class='badge badge-outline-pink font-12'>" . $loop->brand_type . "</span>";
                                                    }
                                                    echo (!empty(array_column(json_decode($loop->brand_name, true), 'value', 'slug')[$_COOKIE['language']])) ?
                                                        array_column(json_decode($loop->brand_name, true), 'value', 'slug')[$_COOKIE['language']] . " - " . $type : $loop->brand_id . " - " . $type; ?>
                                                </td>
                                                <td>
                                                    <?= $loop->model_priority; ?>
                                                </td>
