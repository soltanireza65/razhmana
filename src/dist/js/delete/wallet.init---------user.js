...
receipt.on('change', function () {
    ...
    document.querySelector('#file-name').textContent = receiptFile.name;
...
for (let i = 0; i <withdrawDestination.length ; i++) {
    let id =$(withdrawDestination[i]).prop('id')
    ...
    console.log(withdrawDestination[i])
            ...
            success: function (response) {
                try {
                    ...
                    if (json.status == 200) {
                        ...
                        setTimeout(() => {
                            window.location.href =`/driver/transactions`;
                        }, 3000);
