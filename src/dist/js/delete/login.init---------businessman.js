...
$('button[data-next-step]').on('click', function () {
    if (phone.val().length != 10) {
        sendNotice(lang_vars.alert_warning, lang_vars.login_enter_phone_number, 'warning', 3000);
    } else {
        const params = {
            action: 'login-user',
            ...
            success: function (response) {
                ...
                try {
                    ...
                    if (json.status == 200) {
                        ...
                        sendNotice(lang_vars.alert_warning, lang_vars.login_redirect_driver, 'warning', 5000);
                    } else {
...
function submit() {
    ...
    if (otp.length == 6) {
            ...
            success: function (response) {
                ...
                try {
                    ...
                    if (json.status == 200) {
                        ...
                        sendNotice(lang_vars.alert_error, lang_vars.login_otp_wrong, 'error', 3000);
                    }
                } catch (e) {
                    sendNotice(lang_vars.alert_error, lang_vars.login_unknown_error, 'error', 5000);
                }
        ...
        sendNotice(lang_vars.alert_warning, lang_vars.login_enter_otp, 'warning', 3000);
    }
}