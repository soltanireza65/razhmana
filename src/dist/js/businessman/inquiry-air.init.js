...
const cargoDischarge = $('#cargo-discharge');
        ...
        $.ajax({
            ...
            success: function (response) {
                try {
                    ...
                    json.response.forEach(function (item) {
                        html += `
                        <option value="${item.airport_id}">${item.airport_name} ${item.airport_nameEN}</option>
                        `;
                    });
...
function checkStep2() {
    if (cargoNeededCar.val() <= 0 || cargoNeededCar.val() == '') {
        ...
        sendNotice(lang_vars['alert_warning'], lang_vars['b_inquiry_airport_source_error'], 'warning', 2500);
        ...
        sendNotice(lang_vars['alert_warning'], lang_vars['b_inquiry_airport_dest_error'], 'warning', 2500);
    } else {
        formStepsNum++;
        updateFormSteps();
        updateProgressbar();
...
$('#submit-cargo').on('click', function () {
    let P = cargoRecommendedPrice.val().trim()
    let U = cargoMonetaryUnit.val().trim()
    if ((P.length <= 0 && U == -1) || (P.length > 0 && U != -1)) {
        // run code
        ...
        const params = {
            ...
            cargoDischarge: cargoDischarge.is(':checked'),
            cargoRecommendedPrice: $('#cargo-recommended-price').val().replaceAll(',', ''),
            cargoMonetaryUnit: $('#cargo-monetary-unit').val(),
                ...
                try {
                    ...
                    if (json.status == 200) {
                       ...
                       <h6>${lang_vars['b_tell_inquiry_ground']}</h6>
                        ...
                        setTimeout(() => {
                            window.location.href ='/user/air/inquiry-list';
                        }, 7500);
                    } else {
                        const html = `
        ...
        if (P.length <= 0 && U != -1) {
            ...
            sendNotice(lang_vars['alert_warning'], lang_vars['b_cargo_monetary_value_price'], 'warning', 2500);
        } else {
            // (P.length>0 && U == -1)
...
/*$('#cargo-weight,#cargo-volume').on('input', function () {
