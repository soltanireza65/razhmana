...
$.fn.editableform.buttons = '<button type="submit" class="btn btn-primary editable-submit btn-sm waves-effect waves-light"><i class="mdi mdi-check"></i></button><button type="button" class="btn btn-danger editable-cancel btn-sm waves-effect"><i class="mdi mdi-close"></i></button>',
...
}), $('#change_inquiry_price_value').editable({
    type: 'number',
    pk: 6,
    emptytext: temp_lang.a_empty,
    success: function (response, newValue) {
        let Value = newValue.trim();
        let type = $('#change_inquiry_price_value').data('tj-type');
        if (Value.length > 0) {
            f(type, Value)
        } else {
            toastNotic(temp_lang.error, temp_lang.empty_input);
...
}), $("#change_inquiry_currency_value").editable({
    // prepend: "not selected",
    type: 'selected',
    mode: "inline",
    pk: 7,
    ...
    success: function (response, newValue) {
        let Value = newValue.trim();
        let type = $('#change_inquiry_currency_value').data('tj-type');
        if (Value.length > 0) {
            f(type, Value)
        } else {
            toastNotic(temp_lang.error, temp_lang.empty_input);
    ...
    success: function (response, newValue) {
        // console.log(newValue);
        let Value = newValue.trim();
        let type = $('#add_inquiry_waybill').data('tj-type');
        if (Value.length > 0) {
            f(type, Value)
        } else {
            toastNotic(temp_lang.error, temp_lang.empty_input);
...
}), $('#change_inquiry_contract').editable({
    type: 'text',
    pk: 12,
    emptytext: temp_lang.a_empty,
    success: function (response, newValue) {
        // console.log(newValue);
        let Value = newValue.trim();
        let type = $('#change_inquiry_contract').data('tj-type');
        if (Value.length > 0) {
            f(type, Value)
        } else {
            toastNotic(temp_lang.error, temp_lang.empty_input);
...
$('.btnSubmit').click(function () {
    ...
    if (parseInt(inquiryId) > 0) {
        ...
        let data = {
            action: 'inquiry-air-info-statue',
            inquiryId: inquiryId,
            status: status,
            token: token,
        };
...
function f2(type, newValue) {
    ...
    let data = {
        action: 'inquiry-air-info-desc-admin',
        value: newValue,
        type: type,
        inquiryId: inquiryId,
        token: token,
...
$('#selectCountry').on('select2:select', function (e) {
    ...
    $('#selectCustoms').html('');
    let data = {
        action: 'inquiry-info-location-airport',
        country: country,
    };
...
$('#submitModal').click(function () {
    ...
    let data = {
        action: 'inquiry-air-set-new-location',
        inquiryId: inquiryId,
        country: country,
        city: city,
        customs: customs,
