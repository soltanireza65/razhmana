...
let postCategries = $('#typeCategries').select2().select2('val');

$('.setSubmitBtn').click(function () {
    let id = $(this).attr('id');
    let token = $('#token').val().trim();
    let typeCategries = $('#typeCategries').select2().select2('val');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (myArray[0] == 'successful') {
                    ...
                    window.setTimeout(
                        function () {
                            window.location.replace("/admin/department/edit/" + myArray[1]);
                        },
                        2000
                    );
                } else if (data == "empty") {
