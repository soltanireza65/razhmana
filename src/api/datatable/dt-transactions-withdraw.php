...
if (isset($_SESSION['dt-transactions-withdraw']) && $_SESSION['dt-transactions-withdraw'] == "dt-transactions-withdraw-44") {
//print_r($_POST);
    $query1 = "SELECT COUNT(*) AS allcount FROM `tbl_transactions` INNER JOIN `tbl_users` ON tbl_transactions.user_id=tbl_users.user_id where tbl_transactions.transaction_type='withdraw' ";
    $query2 = "SELECT COUNT(*) AS allcount FROM `tbl_transactions` INNER JOIN `tbl_users` ON tbl_transactions.user_id=tbl_users.user_id where tbl_transactions.transaction_type='withdraw' ";
    $query3 = "SELECT * FROM `tbl_transactions` INNER JOIN `tbl_users` ON tbl_transactions.user_id=tbl_users.user_id  where tbl_transactions.transaction_type='withdraw' ";
    getData123($query1, $query2, $query3, ["transaction_id", "user_firstname", "user_lastname", "transaction_authority", "transaction_tracking_code", "transaction_status", "transaction_date", "transaction_amount", "card_id"], ["railroad_name", "city_name", "railroad_priority", "railroad_id"]);
