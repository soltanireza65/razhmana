...
if (isset($_SESSION['dt-admin-log']) && $_SESSION['dt-admin-log'] == "dt-admin-log-44") {

    $id=0;
    if(isset($_REQUEST['id'])){
        $id= (int) $_REQUEST['id'];
    }
    $query1 = "SELECT COUNT(*) AS allcount FROM tbl_admin_logs WHERE admin_id={$id}";
    $query2 = "SELECT COUNT(*) AS allcount FROM tbl_admin_logs WHERE admin_id={$id}  ";
    $query3 = "SELECT * FROM tbl_admin_logs WHERE admin_id={$id}";
    getData123($query1, $query2, $query3, ["log_detail", "log_os", "log_browser"], ["log_date", "log_detail", "log_os", "log_browser", "log_browser_version"]);
