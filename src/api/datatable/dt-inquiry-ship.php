...
//        $freight_start_date = '<bdi>' . Utils::getTimeCountry($Settings["date_format"], $row["freight_start_date"]) . '</bdi>';
        ...
        $id = '<a target="_self"
                     href="/admin/inquiry/ship/' . $row['freight_id'] . '"
        ...
        $data[] = array(
            ...
            "source_port_id" => $source_country,
            "dest_port_id" => $dest_country,
...
if (isset($_SESSION['dt-inquiry-ship']) && $_SESSION['dt-inquiry-ship'] == "dt-inquiry-ship-44") {

    $query1 = "SELECT COUNT(*) AS allcount FROM tbl_freight_ship WHERE 1 ";
    $query2 = "SELECT COUNT(*) AS allcount FROM tbl_freight_ship WHERE 1 ";
    $query3 = "SELECT * FROM tbl_freight_ship WHERE 1 ";

    getData123($query1, $query2, $query3, ["freight_id", "freight_name", "source_city_id", "dest_city_id", "source_port_id", "dest_port_id", "freight_status", "currency_id"], ["freight_name", "freight_submit_date", "freight_start_date", "freight_id", "freight_status"]);
