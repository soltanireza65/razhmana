...
if (isset($_SESSION['dt-poster-user']) && $_SESSION['dt-poster-user'] == "dt-poster-user-44") {

    $id = 0;
    if (isset($_REQUEST['id'])) {
        $id = (int)$_REQUEST['id'];
    }
    $query1 = "SELECT COUNT(*) AS allcount FROM `tbl_poster` WHERE user_id={$id} ";
    $query2 = "SELECT COUNT(*) AS allcount FROM `tbl_poster` WHERE user_id={$id} ";
    $query3 = "SELECT * FROM `tbl_poster` WHERE user_id={$id} ";
    getData123($query1, $query2, $query3, ["poster_id", "poster_type", "poster_submit_date", "poster_status", "poster_expire"], ["log_date", "log_detail", "log_os", "log_browser", "log_browser_version"]);
